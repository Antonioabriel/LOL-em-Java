...
public class JarvanIV extends Tank{
	


	public JarvanIV(String nome, int vida, int mana, int dano, int armadura,boolean atacou,boolean cura,boolean viva) {
	...
	public void habilidade1(Personagem p) {
		if(!p.isViva()) {
			...
			p.setVida(p.getVida() - this.getDano());
			if(p.getVida() <=0) {
				p.setViva(false);
				p.setVida(0);
	...
	public void habilidade2(Personagem p) {
		if(!p.isViva()) {
			...
			p.setArmadura(p.getArmadura() + 30);;
			if(p.getVida() <=0) {
				p.setViva(false);
				p.setVida(0);
	...
	public void habilidade3(Personagem p) {
		...
		p.setVida(p.getVida() - (this.getArmadura() + this.getDano()));
		if(p.getVida() <=0) {
	...
	public String nomehabilidade1() {
		...
		return "[1] Ataque do Dragao";
	}

	@Override
	public String nomehabilidade2() {
		// TODO Auto-generated method stub
		return "[2] Egide de Ouro";
	}
